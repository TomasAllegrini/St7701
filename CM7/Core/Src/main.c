...
/* USER CODE BEGIN PFP */
static void UART_DebugMessage(const char *msg);
...
static void UART_DebugMessage(const char *msg)
{
  ...
  if ((msg == NULL) || (huart6.gState == HAL_UART_STATE_RESET))
  {
    return;
  }
  ...
  if (len > 0U)
  {
    (void)HAL_UART_Transmit(&huart6, (const uint8_t *)msg, len, 100U);
  }
}
...
int ST7701_SmokeTest_Min(void)
{
  ...
  UART_DebugMessage("[ST7701] Smoke start\r\n");

  panel_hw_reset();

  if (HAL_DSI_Start(&hdsi) != HAL_OK) {
    UART_DebugMessage("[ST7701] HAL_DSI_Start FAIL\r\n");
    return 0;
  }
  ...
  if (ids_are_valid(id1, id2, id3)) {
    UART_DebugMessage("[ST7701] OK: panel responde (IDs validos)\r\n");
    return 1;
  }

  UART_DebugMessage("[ST7701] NO CONFIRMADO: IDs 00/FF (puede ser lectura/BTA o panel)\r\n");
  return 0;
}
